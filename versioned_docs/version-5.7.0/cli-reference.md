...
| `defaultInsertMethod` | string | `plain_query`, `native`, `columnar`, or `plain_bulk`. |
...
# TPC-B with a duration knob
stroppy run tpcb/tx -e duration=5m

# TPC-DS schema load + query suite
...
### Scenario tuning: prefer env knobs

Built-in workloads define `options.scenarios`, so tune concurrency and length with the `VUS`, `DURATION`, `ITER`, and `MAX_DURATION` environment variables &mdash; not the k6 `--vus` / `--duration` / `--iterations` shortflags. Passed after `--`, those shortflags overwrite the workload's scenario block entirely (including `maxDuration`). They still work for backwards compatibility, but the env knobs are the supported way:

```bash
# 10 VUs for 5 minutes (throughput run)
stroppy run tpcc/tx -e vus=10 -e duration=5m

# Fixed number of iterations (power run)
stroppy run tpcb/tx -e iter=100
```

See [Built-in Workloads](./presets#run-knobs-vus-duration-iterations) for the full knob set.
