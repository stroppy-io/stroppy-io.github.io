...
| `defaultInsertMethod` | string | script or preset | `native`, `columnar`, `plain_bulk`, or `plain_query`. Overrides each InsertSpec method when set. |
...
PostgreSQL supports `native`, `columnar`, `plain_bulk`, and `plain_query` inserts. `native` uses pgx `CopyFrom`; `columnar` passes one array per column and lets `unnest` expand it back to rows, binding one parameter per column instead of per row &mdash; this clears PostgreSQL's 65535 bind-parameter limit on wide tables and loads roughly 2.5&ndash;3&times; faster than `plain_bulk` while staying an ordinary `INSERT`; `plain_bulk` uses multi-row INSERT; `plain_query` uses one-row INSERT batches.
...
YDB supports `native`, `columnar`, `plain_bulk`, and `plain_query`. `native` uses YDB Table API `BulkUpsert`; `columnar` is accepted and redirected to the same `BulkUpsert` (already a struct-of-arrays, limit-free payload) with a one-time warning; SQL inserts use the generic `database/sql` path.
...
```bash
stroppy run tpcb/tx -d noop -e vus=4 -e duration=30s
```
...
| `columnar` InsertSpec | `unnest` INSERT | No | No | &rarr; BulkUpsert | Generator drain | No |
...
With no script argument, `stroppy probe` prints the supported insert methods per driver (the matrix above); see [Probe & Script Parameters](./probe#catalog-and-insert-methods).
