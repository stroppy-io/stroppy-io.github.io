...
| `InsertMethod.COLUMNAR` | One array parameter per column, expanded back to rows by the database (PostgreSQL `unnest`). Binds parameters per column, not per row, so it clears the 65535 bind-parameter limit on wide tables. On YDB it redirects to native `BulkUpsert`. |
