...
K6_WEB_DASHBOARD_EXPORT=reports/00-baseline.html \
  stroppy run tpcc/tx -e DURATION=10m
```
...
K6_WEB_DASHBOARD_EXPORT="$REPORT_PATH" \
  stroppy run "$WORKLOAD" -e DURATION="$DURATION"
...
Beyond standard k6 metrics, Stroppy tracks the following. These are the k6-internal metric names, as they appear in the web dashboard and HTML report. When the same data is exported over OpenTelemetry each name gains a `stroppy_` prefix and an instrument suffix (`_total`, `_milliseconds_bucket`, `_bytes_total`) and the scenario becomes a label &mdash; see [Metric names](#metric-names) below.
...
Through OpenTelemetry, Stroppy metric names are prefixed with `stroppy_` and suffixed by instrument type, and the scenario is a label rather than baked into the name:

| Instrument | Suffix | Example |
|------------|--------|---------|
| Counter | `_total` | `stroppy_insert_rows_total`, `stroppy_data_received_bytes_total` |
| Histogram (duration) | `_milliseconds_bucket` | `stroppy_run_query_duration_milliseconds_bucket` |
| Counter (bytes) | `_bytes_total` | `stroppy_data_sent_bytes_total` |

Each metric carries a `scenario` label (the workload scenario name) and the collector emits the service under a `job` label, not `service.name`. Build dashboard queries and alert rules against these names.

### Grafana dashboard

Stroppy ships a ready-made Grafana dashboard at [`docs/dashboard.json`](https://github.com/stroppy-io/stroppy/blob/main/docs/dashboard.json) that plots load-phase throughput, query latency, and transaction mix from the OTLP stream. Import it into Grafana and point it at your collector. The "Insert rows/s" panels filter `event="progress"` so they track the live row counter during a load.
