...
stroppy run tpcds -d pg            # TPC-DS schema load + query suite
```

### 5. Set concurrency and duration

Tune a run with the `VUS`, `DURATION`, and `ITER` environment variables:

```bash
stroppy run tpcb/tx -e vus=10 -e duration=30s
```
...
- [Tests](tests/tpcc) &mdash; TPC-C, TPC-H, and TPC-DS workload details.
